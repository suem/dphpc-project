...
\usepackage{spconf,amsmath,amssymb,graphicx,epstopdf}
...
Maximum cardinality matching, focus on existing algorithms and optimize the parallel versions in a highly multithreaded environment. Focus on Pothen-Fan, reason about performance.
\end{abstract}
...
\mypar{Motivation} Graph matching has several applications in computer science, for example the marriage problem or computing the block triangular form (BTF) of a sparse matrix \cite{Pothen:1990}. As data gets bigger, the performance of algorithms that solve these problems gets more and more important. 

\mypar{Related work} \cite{Azad:2012} \cite{Azad:2015} 
...
Paper \cite{Azad:2015} claims that PPF is not well suited for many thin cores.

\mypar{Parallel Tree Grafting} 
Pseudocode for parallel tg

\mypar{Optimizations}
Same optimizations as PPF version 3 for the augmenting paths. Non-blocking queue as a data structure. 
