...
inline bool lookahead_step(
	...
	for (std::tie(laStart, laEnd) = lookahead[x0]; laStart != laEnd; ++laStart) {
		Vertex y = *laStart;
        if (mate[y].mate == Graph::null_vertex() && mate[y].iteration < iteration) {
            if (std::atomic_exchange(&mate[y].iteration, iteration) < iteration) {
				// update matching
				mate[y].mate = x0;
...
inline bool dfs_la_atomic(
	...
	bool init_lookahead_success = lookahead_step(v, g, first_right, mate, iteration, lookahead);
	...
	while(!stack.empty()) {
		...
		if (yiter != yiter_end) { // if there are still neighbours to visit
			Vertex y = *yiter;
			Vertex x1 = mate[y].mate;

			bool lookahead_success = lookahead_step(x1, g, first_right, mate, iteration, lookahead);
...
void ppf4(const Graph& g, Vertex first_right, std::vector<MateVisited>& mate, int numThreads) {
    ...
    for (Vertex v = 0; v < first_right; v++) {
        ...
        if (mate[v].mate == Graph::null_vertex()) {
	...
	do {
        ...
        if (iteration == 0) { // integer overflow on iteration, reset visited flags
            iteration = 1;
            for (auto& mv : mate) mv.iteration = 0;
        }
...
#pragma omp for
			for (int i = 0; i < unmatched_size; i++) {
				...
				bool path_found_v = dfs_la_atomic(v, g, first_right, mate, iteration, lookahead, stack);
				if (path_found_v) {
					urv.unmatched = false;
...
#pragma omp for
    ...
    for (int y = first_right; y < n; y++) {
		if (mate[y].mate != Graph::null_vertex()) {
			mate[mate[y].mate].mate = static_cast<int>(y);
		}
	}
}
